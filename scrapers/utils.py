...
def parse_spanish_date_text(text:str, default_year=None):
    """
    Parse flexible Spanish date expressions used in Málaga sources.
    Returns: (date_start: date, date_end: date, occurrences: list[datetime or date], all_day: bool)
    Supports:
      - 'Del 01 de abril al 14 de septiembre de 2025'
      - 'Del 04 al 11 de septiembre de 2025'
      - '22 de septiembre de 2025'
      - '03/07/2025 – 31/01/2027'
      - '21/02/2025 — 02/2026'  -> 2025-02-21 to 2026-02-28
    ...
    # DD/MM/YYYY - DD/MM/YYYY or DD/MM/YYYY - MM/YYYY
    ...
    if m:
        ...
        end_day = last_day_of_month(y2,m2)
        return date(y1,m1,d1), date(y2,m2,end_day), [], True

    # 'del DD de mes al DD de mes de YYYY' or 'del DD al DD de mes de YYYY'
    ...
    if m:
        ...
        if month1:
            ...
            # like "Del 04 al 11 de septiembre de 2025" -> assume same month as month2
            m1 = MONTHS_ES[month2]
        return date(y,m1,_norm_day(d1)), date(y,MONTHS_ES[month2],_norm_day(d2)), [], True

    # 'dd de mes de yyyy' (single day)
    ...
    # 'mes yyyy - mes yyyy' OR 'mes - mes yyyy'
    ...
    # 'dd mes — dd mes yyyy' or 'dd, dd, dd mes yyyy' lists
    # lists like '1, 8, 15, 22 y 29 octubre 2025'
    m = re.match(r"([\d,\syy]+)\s+([a-záéíóú]+)\s+(\d{4})", t)
    ...
    # fallback: try dateutil parse start only
    try:
